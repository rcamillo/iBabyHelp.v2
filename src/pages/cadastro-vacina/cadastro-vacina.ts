...
export class CadastroVacinaPage {
  ...
  private setupPageTitle() {
    this.title = this.navParams.data.vacina
      ? "Alterando dados de Vacina"
      : "Nova Vacina";
  }
  ...
  onSubmit() {
    if (this.form.valid) {
      this.provider
        ...
        .then(() => {
          this.toast
            .create({ message: "Vacina salva com sucesso.", duration: 3000 })
            .present();
          this.navCtrl.pop();
        })
        .catch(e => {
          this.toast
            .create({ message: "Erro ao salvar Vacina.", duration: 3000 })
            .present();
          console.error(e);
        });
    }
